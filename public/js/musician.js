...
function Redirect() {
    $.ajax({
        method: 'GET',
        url: 'http://localhost/symphony/serviceproviders/fetchmusicians',
        dataType: 'json',
    ...
    if (inventory && inventory.length > 0) {
        inventory.forEach(function (item) {
            ...
            req += `<div class="item-container">` +
                ...
                `<a href="http://localhost/symphony/serviceproviders/viewMusicians/`+item.product_id+`" style="color: orange">see more details</a>` +
                `<p>` + stockText + `</p>` +
                <!-- User reviews go here -->
                `<div class="user-review">` +
                `<a href="http://localhost/symphony/serviceProviders/reviewMusician/` + item.product_id +`" style="font-size: 0.9rem;">Read Customer Reviews</a>` +
                `</div>` +
                `</div>` +
                `</div>` +
                <!-- Review section -->
