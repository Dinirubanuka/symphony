...
function Redirect() {
    ...
    $.ajax({
        method: 'POST',
        url: 'http://localhost/symphony/users/band',
        dataType: 'json',
        success: function (response) {
    ...
    if (selectedCategories.length === 0) {
        ...
        filteredData = JSON.parse(JSON.stringify(Orgdata)).filter(function (item) {
            return selectedCategories.some(function (selectedCategory) {
                ...
                return item.location.includes(selectedCategory) || item.instrument.includes(selectedCategory);
            });
        });
    }
...
function search() {
    ...
    products.forEach(product => {
        ...
        const leaderNameElement = product.querySelector(".item-info h3:nth-of-type(2)");
        ...
        const leaderName = leaderNameElement.textContent || leaderNameElement.innerHTML;
        ...
        const searchValues = [title, leaderName, location, price];
